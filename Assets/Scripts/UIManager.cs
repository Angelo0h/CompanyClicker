...
public class UIManager : MonoBehaviour
{
    ...
    void MapValueToPlayer()
    {
        ...
        {
            player.myStats.state[i] = new Block("UpgradeManager",el.upgradeName, el.cost, el.count);
            //Debug.Log($"#{i}, #{el.name}, #{el.cost}, ${el.count}");
            i++;
        }
        foreach(UpgradePerSec el in upgradePerSecs) 
        {
            player.myStats.state[i] = new Block("UpgradePerSec",el.upgradeName, el.cost, el.count);
            //Debug.Log($"#{i}, #{el.name}, #{el.cost}, ${el.count}");
            i++;
        }
    }
